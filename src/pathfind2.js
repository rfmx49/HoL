...
function easyStarFireRoute() {
	...
	objectMap[currentDoor.y][currentDoor.x] = "FR";
	var firstStep = getInFront("door",currentDoor.y,currentDoor.x);
	var currentPos = {xtile: firstStep.x,ytile: firstStep.y};
	//addThis tile to our fireTable
	objectMap[currentPos.ytile][currentPos.xtile] = "FR";
	//get next door.
	if (doors.length > 0) {
		currentDoor = doors.shift();
		//get next destinations
		x = currentDoor.x;
		y = currentDoor.y;
	}
	else {
	...
	while ((doors.length > 0)) {
		easystar.findPath(currentPos.xtile, currentPos.ytile, x, y, function( path ) {
			if (path === null) {
				...
				//add all nodes to fireRoute
				for (var pathNode = 1; pathNode < path.length; pathNode++) {
					objectMap[path[pathNode].y][path[pathNode].x] = "FR";
				}
			}
		});
		//get nextDoor set new current postion to last door.
		currentPos.xtile = currentDoor.x;
		currentPos.ytile = currentDoor.y;
		currentDoor = doors.shift();
		x = currentDoor.x;
		y = currentDoor.y;
	}
